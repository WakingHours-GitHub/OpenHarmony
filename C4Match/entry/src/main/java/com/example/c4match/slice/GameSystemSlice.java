...
//package com.huawei.c4match.slice;
...
public class GameSystemSlice extends AbilitySlice {
    private static final String TAG = CommonData.TAG + GameSystemSlice.class.getSimpleName();
//    private static final String TAG = this.getClass().getName();
    ...
    private void initView(Intent intent) {
        ...
        if (findComponentById(ResourceTable.Id_text_title) instanceof Text) {
            ...
            textTitle.setText(isLocal ? "拼图游戏(本地端)" : "拼图游戏(远程端)");
        }
    ...
    private void moveFun(int resourceId, int position) {
        Log.info(TAG+ "moveFun, resourceId is " + resourceId + " position is " + position);
        ...
        Log.info(TAG+"blankPosition is " + blankPosition + " num1 is " + num1 + " num2 is " + num2);
        if ((num1 == 0 && num2 == 1) || (num1 == 1 && num2 == 0)) {
            Log.info(TAG+"can move image");
            ...
            Log.info(TAG+"moveFun, come to change blank position");
    ...
    private void setFlagValue(int resourceId, int position) {
        Log.info(TAG+"setFlagValue, resourceId is " + resourceId + " position is " + position);
        int positionX = position / imageX;
        int positionY = position % imageY;
        int blankX = blankPosition / imageX;
        int blankY = blankPosition % imageY;
        ...
        if ((num1 == 0 && num2 == 1) || (num1 == 1 && num2 == 0)) {
            Log.info(TAG+ "setFlagValue, come to change blank position");
            blankPosition = position;
            blankResId = resourceId;
        }
        gameOverFun();
    ...
    private void setImageClickable(boolean isClickable) {
        Log.info(TAG+"setImageClickable and flag is " + isClickable);
        for (int i = 0; i < imageCount; i++) {
...
//                    .withAbilityName(CommonData.PICTURE_GAME_SERVICE_NAME)
                    .withAbilityName(CommonData.GAME_CLASS_NAME)
            ...
            conn = new IAbilityConnection() {
                ...
                public void onAbilityConnectDone(ElementName elementName, IRemoteObject remote, int resultCode) {
                    Log.info(TAG+ "onAbilityConnectDone......");
                ...
                public void onAbilityDisconnectDone(ElementName elementName, int resultCode) {
                    ...
                    Log.info(TAG+ "onAbilityDisconnectDone......");
                }
            };
    ...
    private void connectAbility(ElementName elementName, IRemoteObject remote, int requestType) {
        ...
        Log.error(TAG+ "connectRemoteAbility done");
        if (proxy != null) {
            try {
                proxy.senDataToRemote(requestType);
            } catch (RemoteException e) {
                Log.error(TAG+ "onAbilityConnectDone RemoteException");
            }
        }
    }
    ...
    private void senDataToRemoteFun() {
        if (!isShare) {
            Log.info(TAG+ "should not senDataToRemote");
            return;
        }
        sendTime = System.currentTimeMillis();
        Log.info(TAG+ "senDataToRemote, and sendTime is " + sendTime);
        if (isConnected && proxy != null) {
            try {
                proxy.senDataToRemote(PictureRemoteProxy.REQUEST_SEND_DATA);
            } catch (RemoteException e) {
                Log.info(TAG+ "processEvent RemoteException");
            }
        } else {
            Log.info(TAG+ "processEvent RemoteException Error");
        }
    }
    ...
    private void getDevices() {
        ...
        Log.info(TAG+ "deviceInfos size is :" + deviceInfos.size());
    ...
    private void updateDataInfo(Intent intent) {
        ...
        Log.info(TAG+ "receive moveImageId:" + moveImageId);
        Log.info(TAG+ "receive movePosition:" + movePosition);
        getUITaskDispatcher().delayDispatch(this::setImages, DELAY_TIME);
    }

    private void setImages() {
        Log.info(TAG+ "setImages");
    ...
    private void subscribe() {
        ...
        try {
            Log.info(TAG+CommonData.GAME_CLASS_NAME+" subscribeCommonEvent");
            CommonEventManager.subscribeCommonEvent(subscriber);
        } catch (RemoteException e) {
            Log.error(TAG+"subscribeCommonEvent occur exception.");
        }
    }
    ...
    private void unSubscribe() {
        try {
            ...
            Log.error(TAG+ "unSubscribe Exception");
        }
    }

    private void pictureRandom() {
        Log.info(TAG+ "pictureRandom start");
        ...
        if (inverseNumber(imageIndexs) % DIVIDED_NUM2 == 0) {
            ...
            Log.info(TAG+ "pictureRandom end");
        } else {
            Log.info(TAG+ "pictureRandom failed");
            pictureRandom();
        }
    }
    ...
    class PictureRemoteProxy implements IRemoteBroker {
        ...
        private void senDataToRemote(int requestType) throws RemoteException {
            ...
            try {
                ...
                if (ec != ERR_OK) {
                    Log.error(TAG+ "ec != ERR_OK RemoteException");
                }
            } catch (RemoteException e) {
                Log.error(TAG+ "RemoteException");
            } finally {
                data.reclaim();
                reply.reclaim();
            }
    ...
    class MyCommonEventSubscriber extends CommonEventSubscriber {
        ...
        public void onReceiveEvent(CommonEventData commonEventData) {
            ...
            Log.info(TAG+ "onReceiveEvent, sendTime - reciveTime is " + (sendTime - reciveTime));
            if (Math.abs(sendTime - reciveTime) <= MAX_RECIVE_TIME) {
                Log.info(TAG+ "almost at the same time, do not handle recive msg");
                isShare = false;
